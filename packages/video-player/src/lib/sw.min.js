          ...
          ? t.includes(self.registration.scope + 'webtorrent/keepalive/')
            ...
            : (async function ({ request: s }) {
                const { url: t, method: n, headers: o, destination: a } = s,
                  l = await clients.matchAll({ type: 'window', includeUncontrolled: !0 }),
                  [r, i] = await new Promise(e => {
                    for (const s of l) {
                      ...
                      (r.onmessage = ({ data: s }) => {
                        ...
                        s.postMessage({ url: t, method: n, headers: Object.fromEntries(o.entries()), scope: self.registration.scope, destination: a, type: 'webtorrent' }, [i]);
                    }
                  });
                let c = null;
                const d = () => {
